...
export class GroupMessageService {
  ...
  async create(createGroupMessageDto: CreateGroupMessageDto) {
    const isUser = await this.groupChatUserService.selectUser(createGroupMessageDto.fromUserId);
    if (!isUser) {
      throw new BadRequestException('此用户不在群里中');
    }
    const res = this.groupMessageRepository.create(createGroupMessageDto);
    ...
    //   .orWhere('(group-message.createdTime > gcu.exitTime AND group-message.fromUserId <> gcu.userId)')
    //   .groupBy('gcu.userId')
    //   .getRawMany()) as { userId: number; msgNumber: number }[];

    // // 正在群中聊天用户
    // const allRedisList = await this.redisService.getAllActiveUser();
    // const allRedisListParse = allRedisList.map((item: string) => JSON.parse(item)) as {
    //   msgType: MessageEnum;
    //   userId: number;
    //   toUserId: number;
    // }[];
    ...
    // if (allRedisListParse.filter(item => item.toUserId === createGroupMessageDto.groupId).length > 1) {
  ...
  async findAll(id: number | string, listGroupMessage: ListGroupMessageDto, currentUser?: number) {
    ...
    if (listGroupMessage.createdTime && listGroupMessage.createdTime.length) {
      queryBuilder.andWhere('DATE_FORMAT(group-message.createdTime, "%Y-%m-%d") BETWEEN :startDate AND :endDate', {
  ...
  async updateGroupUserMsgNumber(groupChatId: number, msgNumber: number, userId?: number) {
    this.groupChatUserService.updateSingleMsgNumber(groupChatId, msgNumber, userId);
  }
  ...
  async upDateEnterExitTime(userId: number, enterExitTime: EnterExitTime) {
    return await this.groupChatUserService.updateEnterTimeExitTime(userId, enterExitTime);
  }
